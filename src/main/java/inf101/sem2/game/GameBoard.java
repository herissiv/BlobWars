...
public class GameBoard {
	...
	/**
	 * Shall check every neighbour with max distance 2, and check if that location is the current player
	 * @param to
	 * @param elem
	 * @return
	 */
	public List<Location> neighbourLocations(Location to, Player elem){
		List<Location> neighbours = new ArrayList<>();
		if (this.grid.neighbourLocations(to, elem) != null){
			neighbours = this.grid.neighbourLocations(to, elem);
		}
		return neighbours;
	}
}
