...
public class Grid<T> implements IGrid<T> {
	...
	@Override
	public List<Location> neighbourLocations(Location to, T elem) {
		// TODO Auto-generated method stub
		List<Location> neighBourList = new ArrayList<>();
		for (int row = to.row-2; row<= to.row + 2; row++){
            for (int col = to.col-2; col<= to.col+2; col++){
                if (row == to.row && col == to.col) {
                    continue;
                }
                else {
                    try {
                        if (get(new Location(row, col))==elem){
                            neighBourList.add(new Location(row, col));
                        }
